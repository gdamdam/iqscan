...
if(p.kind==='waterfall'){lines.unshift('Time '+hms(yv)+' from scan interval start');decode(p);if(p.levels){let q=p.levels;let row=p.time_bin_s?Math.floor(Math.max(0,yv-(p.time_origin_s||0))/p.time_bin_s):Math.floor(v*q.full_rows);row=Math.min(q.rows-1,Math.max(0,Math.floor(row/q.row_step)));let col=Math.min(q.cols-1,Math.floor(u*q.full_cols/q.col_step));let level=q.view.getInt16(2*(row*q.cols+col),true)/10;lines.push('Approx. level '+level.toFixed(1)+' dB above reference band (sampled; not SNR)');}
